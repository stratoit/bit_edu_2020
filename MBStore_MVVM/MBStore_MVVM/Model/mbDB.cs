...
namespace MBStore_MVVM.Model
{
    class mbDB
    {
        ...
        #region 거래내역
        //거래내역 - 물품목록 출력
        public List<Product> Get_Lo_Rse_ProductList(int productid, int tradehistoryID, string color, string name, DateTime startDay, DateTime endDay, string query)
        {
            ...
            try
            {
                using (conn = new SqlConnection())
                {
                    ...
                    SqlParameter param_name = new SqlParameter("@ProductName", "%" + name + "%");
        ...
        #region 제품조회
        //제품조회 - 제품목록 출력
        public List<Product> Select_Lo_Pse_Product(int stockid, int productid, string productname, string color, string query)
        {
            List<Product> proList = new List<Product>();
            ...
            using (conn = new SqlConnection())
            {
                conn.ConnectionString = ConfigurationManager.ConnectionStrings["UserDB"].ToString();
                conn.Open();    //  데이터베이스 연결     
                string sql = "select sp.stock_product, sp.product_id, p.name, sp.color, sp.color_value, sp.stock from stock_product as sp, product as p where sp.product_id = p.product_id and sp.stock > 0 " + query;

                SqlCommand cmd = new SqlCommand(sql, conn);

                SqlParameter param_historyid = new SqlParameter("@StockID", stockid);
                param_historyid.SqlDbType = System.Data.SqlDbType.Int;
                cmd.Parameters.Add(param_historyid);

                SqlParameter param_id = new SqlParameter("@ProductID", productid);
                param_id.SqlDbType = System.Data.SqlDbType.Int;
                cmd.Parameters.Add(param_id);

                SqlParameter param_name = new SqlParameter("@ProductName", "%" + productname + "%");
                param_name.SqlDbType = System.Data.SqlDbType.VarChar;
                cmd.Parameters.Add(param_name);
        ...
        public List<string> Get_Lo_Input_ColorList(string pid)
        {
            List<string> productList = new List<string>();
            try
            {
                using (conn = new SqlConnection())
                {
                    ...
                    string sql = "select sp.color from product as p, stock_product as sp where sp.product_id = p.product_id and p.product_id = " + pid + " order by p.product_id asc";
        ...
        //입고된 제품목록 출력
        public ObservableCollection<Product> Select_Lo_Pse_stockProduct(string query)
        {
            ObservableCollection<Product> proList = new ObservableCollection<Product>();
        ...
        //입고 : 트랜잭션 함수
        public bool input_transaction(ObservableCollection<Product> inputdata, bool[] check, int[] newstock)
        {
            try
            {
                using (conn = new SqlConnection())
        ...
        //public List<Employee> GetList_Emp_info() //지원-> 직원조회 -> 직원 전체출력
        {
